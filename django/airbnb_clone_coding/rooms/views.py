...
class Rooms(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if not category_pk:
                raise ParseError("category가 필요합니다.")
            try:
                category = Category.objects.get(pk=category_pk)
                if category.kind == Category.CategoryKindChoices.EXPERIENCES:
                    raise ParseError("category의 kind가 'rooms'여야 합니다.")
...
class RoomReviews(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            page = request.query_params.get("page", 1) # 디폴트는 1로 설정한다
            page = int(page) # page는 str으로 인식되므로 int로 설정
        except ValueError:
            page = 1
